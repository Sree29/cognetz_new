...
const SingleFeatureV1 = ({ feature }: { feature: DataType }) => {
    const { id, thumb, projectName, subTitle1, subTitle2, info, date, website } = feature;
    const { containerRef, hoverElementRef } = useHoverEffect();

    const targetUrl = website || `/project-details/${id}`;
    const isExternal = !!website;

    return (
        <div className="feature-project" ref={containerRef}>
            <div className="hover_mouse" ref={hoverElementRef}>
                <Link href={targetUrl} target={isExternal ? "_blank" : "_self"} rel={isExternal ? "noopener noreferrer" : undefined}>
                    View
                </Link>
            </div>
